...
export const Record = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      const dummyData = Array.from({ length: 64 }).map((_, index) => ({
        // 데이터량 설정
        id: index + 1,
    ...
    const filtered = data.filter((item) => {
      ...
      return (
        (selectedButtons.length === 0 ||
          selectedButtons.includes(item.result)) &&
    ...
    if (totalRecords === 0) {
      ...
      return (
        <tr>
          <td
            colSpan="4"
            style={{ textAlign: "center", fontSize: "1.5vh", color: "#3A278B" }}
          >
              ...
              <Button
                ...
                onClick={() =>
                  setSelectedButtons((prevState) =>
                    prevState.includes(result)
                      ? prevState.filter((id) => id !== result)
                      : [...prevState, result]
                  )
                }
              >
